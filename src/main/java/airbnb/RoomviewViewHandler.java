...
public class RoomviewViewHandler {
    ...
    //////////////////////////////////////////////////
    // Complain 이 등록되었을 때 Update (결재된 경우만 Complain 가능하므로 Update. Room 에 대해 1번 가능) -> RoomView TABLE
    //////////////////////////////////////////////////
    @StreamListener(KafkaProcessor.INPUT)
    public void whenComplainRegistered_then_CREATE_1 (@Payload ComplainRegistered complainRegistered) {
        try {
            if (!complainRegistered.validate()) return;
            
            /*
            Roomview roomview = new Roomview();
            
            roomview.setId(complainRegistered.getCmpId());
            roomview.setContents(complainRegistered.getContents());                        
            
            roomviewRepository.save(roomview);
            */

            System.out.println("#######################");
            System.out.println("###complainRegistered1 ###" + complainRegistered.getRoomId());
            System.out.println("###complainRegistered2 ###" + complainRegistered.getCmpId());
            System.out.println("#######################");

            List<Roomview> roomviewList = roomviewRepository.findByPayId(complainRegistered.getPayId());
            
            for(Roomview roomview : roomviewList){
                // view 객체에 이벤트의 eventDirectValue 를 set 함
                roomview.setCmpId(complainRegistered.getCmpId());                
                roomview.setContents(complainRegistered.getContents());
                // view 레파지 토리에 save
                roomviewRepository.save(roomview);
            }
    ...
    //////////////////////////////////////////////////
    // Complain 이 수정되었을 때 Update -> RoomView TABLE
    //////////////////////////////////////////////////
    @StreamListener(KafkaProcessor.INPUT)
    public void whenComplainModified_then_UPDATE_1(@Payload ComplainModified complainModified) {
        try {
            if (!complainModified.validate()) return;
                
            Optional<Roomview> roomviewOptional = roomviewRepository.findById(complainModified.getCmpId());

            if( roomviewOptional.isPresent()) {
                Roomview roomview = roomviewOptional.get();
                
                roomview.setContents(complainModified.getContents());                                
                roomviewRepository.save(roomview);
            }
    ...
    //////////////////////////////////////////////////
    // Complain 이 삭제 되었을 때 Delete -> RoomView TABLE
    //////////////////////////////////////////////////
    @StreamListener(KafkaProcessor.INPUT)
    public void whenComplainDeleted_then_DELETE_1(@Payload ComplainDeleted complainDeleted) {
        try {

            if (!complainDeleted.validate()) return;
            
            roomviewRepository.deleteById(complainDeleted.getCmpId());
